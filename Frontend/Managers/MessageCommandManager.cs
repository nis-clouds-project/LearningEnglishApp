...
public static class MessageCommandManager
{
    private static readonly ILogger Logger = LoggerFactory.Create(builder => builder.AddConsole())
        .CreateLogger("MessageCommandManager");
    ...
    public static async Task HandleStartCommand(ITelegramBotClient botClient, Message message, CancellationToken cancellationToken)
    {
        ...
        try
        {
            Logger.LogInformation("Processing /start command for user {ChatId}", chatId);
        ...
        {
            Logger.LogError(ex, "Error processing /start command for user {ChatId}", chatId);
            await botClient.SendTextMessageAsync(
                chatId: chatId,
